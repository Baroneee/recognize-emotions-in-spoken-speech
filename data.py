...
if not os.path.exists('sentiment_data.csv'):
    raise FileNotFoundError("File sentiment_data.csv chưa tồn tại. Hãy chạy create_data.py trước.")
data = pd.read_csv('sentiment_data.csv').dropna()
texts = data['text'].tolist()
labels = data['label'].map({'Positive': 0, 'Negative': 1, 'Neutral': 2}).tolist()

# Tokenize văn bản đã được xử lý
tokenized_texts = [word_tokenize(text.lower(), preserve_line=True) for text in texts]

# Tạo từ điển từ các từ đã được tokenize
...
train_texts, test_texts, train_labels, test_labels = sk.train_test_split(
